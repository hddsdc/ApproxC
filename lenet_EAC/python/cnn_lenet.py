...
def conv_net(params, layers, data, labels, doBackProp):
  ...
  output_images = np.reshape(output_images, (output_images.shape[0],1))
  # print(output_images.shape[0])
  # plot_image(output_images, output[1]['width'], output[1]['height'], output[1]['channel'])
  ...
  for i in range(2, l):
    ...
    #   if (i == 2):
        #   print("CONV LAYER")
        #   output_images = output[i]['data']
        #   plot_image(output_images, output[i]['width'], output[i]['height'], output[i]['channel'])
    elif layers[i]['type'] == 'POOLING':
...
def plot_image(image_data, w_in, h_in, c):
    w_mat = image_data.shape[0]
    h_mat = image_data.shape[1]
    print_image = np.reshape(image_data, (w_in, h_in, c))
    w_mat = print_image.shape[0]
    h_mat = print_image.shape[1]
    c = print_image.shape[2]
    #Test Value of C
    #c = 1
    print("Printing Image Pixels")
    for channel in range(0,c):
        image_plot = print_image[:,:,channel]
        cmap2 = mpl.colors.LinearSegmentedColormap.from_list('my_colormap',
                                               ['white','black'],
                                               256)
        # plt.subplot(4,5,channel+1)
        img2 = plt.imshow(image_plot,interpolation='nearest',
                        cmap = cmap2,
                        origin='lower')
    ...
    print("Images Dims are: w: " + str(w_mat) + " h: " + str(h_mat))
    print("Channel Value is " + str(c))
...
def pooling_layer_forward(input, layer):
  ...
  # print(stride , k, h_in, w_in, c, batch_size)
  # print("Printing Data")
  ...
  data = np.reshape(data,(w_in, h_in, c, batch_size))
  output_data = np.reshape(output_data,(w_out, h_out, c, batch_size))
  x_count = 0
  y_count = 0

  for batch_loop in range(0,batch_size):
    # Outer Loop Channels
    for c_loop in range(0,c):
        y_count = 0
    # Y Traverser
        for y_loop in xrange(0,h_in, stride):
            x_count = 0
            for x_loop in xrange(0,w_in, stride):

                max_val = -1
                for y_pool_index in range(y_loop, y_loop + k):
                    if (y_loop + k <= h_in):
                    # X Traverser
                        for x_pool_index in range(x_loop,x_loop + k):
                            if (x_loop + k <= w_in):
                                if data[x_pool_index][y_pool_index][c_loop][batch_loop] > max_val:
                                    max_val = data[x_pool_index][y_pool_index][c_loop][batch_loop]

                output_data[x_count][y_count][c_loop][batch_loop] = max_val
                x_count += 1
            y_count += 1


  # print(input['data'])
  # print(stride , k, h_out, w_out, c, batch_size)
  # implementation ends
  output_data = np.reshape(output_data, (h_out * w_out * c, batch_size))
...
def inner_product_forward(input, layer, param):
  ...
  batch_result = np.zeros((num, batch_size))

  data = input['data']
  ip_shape_x = data.shape[0]
  ip_shape_y = data.shape[1]

  param_w_shape_x = param['w'].shape[0]
  param_w_shape_y = param['w'].shape[1]

  weight_matrix = param['w']
  bias_matrix = param['b']

  param_b_shape_x = param['b'].shape[0]

  for batch_count in range(0, batch_size):
      row_matrix = data[:,batch_count]
    #   row_size = len(row_matrix)
      ip_col_matrix = np.transpose(row_matrix)
      result_matrix = np.dot(ip_col_matrix, weight_matrix)
      sum_matrix = np.add(result_matrix, bias_matrix)
      batch_result[:,batch_count] = sum_matrix
    #   print(row_matrix)
    #   print(row_size)

  # param_b_shape_y = param['b'].shape[1]

  # layer_data = layer['data']
  # layer_shape_x = layer_data.shape[0]
  # layer_shape_y = layer_data.shape[1]
  #
  # print(" IP X=",ip_shape_x, "IP Y=", ip_shape_y, param_b_shape_x, "W Shape X=", param_w_shape_x, "W Shape Y=", param_w_shape_y)
  # # TODO: implement your inner product forward pass here
  # implementation begins
  output['data'] = batch_result
  # implementation ends
