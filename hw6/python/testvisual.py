...
def main():
  ...
  matfile1 = 'lenet_q4.mat'
  matfile2 = 'lenet_full.mat'

  params_30 = pickle.load(open(matfile1, "rb"))
  params_full = pickle.load(open(matfile2, "rb"))
  ...
  xtest_image = xtest[:,0]
  xtest_image = np.reshape(xtest_image, (xtest_image.shape[0],1))
  ytest_image = ytest[0]
  print(xtest_image.shape[0])
  print(xtest_image.shape[1])

  cptest, _ = cnn_lenet.conv_net(params, layers, xtest_image, ytest_image, False)
  ...
  #print 'test_cost = %f test_accuracy = %f' % (cptest['cost'], cptest['percent']) + '\n'
